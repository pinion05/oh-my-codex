...
const KOREAN_CANCEL_KEYWORDS = new Set(['중단', '취소', '멈춰', '그만', '종료']);

function keywordToPattern(keyword: string): RegExp {
  const escaped = escapeRegex(keyword);
  if (KOREAN_CANCEL_KEYWORDS.has(keyword)) {
    return new RegExp(
      `(?<![\\p{L}\\p{N}_])${escaped}(?:(?:\\s*(?:해주세요|해줘|시켜|하자|해))|(?![\\p{L}\\p{N}_]))`,
      'iu',
    );
  }
...
const KOREAN_CANCEL_INTENT_PATTERNS: RegExp[] = [
  /^\s*(?:중단|취소|멈춰|그만|종료)(?:\s*(?:해|해줘|해주세요|시켜|하자))?\s*[.!。]?\s*$/i,
  /^\s*(?:현재\s*)?(?:(?:실행\s*중(?:인)?\s*)?(?:모드|작업|워크플로우|흐름|세션)|omx|오엠엑스|workflow|mode|task|run|execution|deep-interview|ralph|team|ultrawork|autopilot)(?:\s*(?:을|를|은|는))?\s*(?:중단|취소|멈춰|그만|종료)(?:\s*(?:해|해줘|해주세요|시켜|하자))?\s*[.!。]?\s*$/i,
];
...
export function detectKeywords(text: string): KeywordMatch[] {
  ...
  for (const { keyword, pattern, skill, priority } of KEYWORD_MAP) {
    ...
    if (match) {
      if (!hasIntentContextForKeyword(normalizedText, keyword.toLowerCase())) continue;
      implicit.push({
        keyword: match[0],
        skill,
        priority,
